...
from logger_window.logs.search_ast import (
    AndNode,
    CompareNode,
    EmptyNode,
    FieldNode,
...
from logger_window.logs.search_models import (
    AggregateQuery,
    AggregateResult,
    CompareOperator,
    IgnoreRule,
...
from logger_window.logs.search_similarity import DEFAULT_SIMILARITY_THRESHOLD, similarity_score
from logger_window.logs.search_text_analysis import (
    is_time_query,
    parse_query,
    resolve_timezone,
)
from logger_window.logs.traceql_bridge import match_traceql_search, should_use_legacy_search
...
def match_search_query(log: LogDict, query: str | SearchQuery, tz: str | tzinfo) -> bool:
    ...
    if parsed.start is None and parsed.end is None and not should_use_legacy_search(parsed.raw_text):
        try:
            return match_traceql_search(log, parsed.raw_text, tz)
        except Exception:
            pass
